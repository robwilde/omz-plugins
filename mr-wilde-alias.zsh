...
alias cnpm='sudo rm -rf node_modules package-lock.json'
dnpm(){ dbashc "npm $1"; }
dnpmr(){ dnpm "run $1"; }
...
alias dbehat='dbash -c "php vendor/bin/behat --format pretty --format progress --colors --no-interaction --verbose=2 --stop-on-failure"'
dbehatname(){ dbash -c "php vendor/bin/behat --format pretty --format progress --colors --no-interaction --verbose=2 --stop-on-failure --name='$1' $2"; }
dbehatfile(){ dbash -c "php vendor/bin/behat --format pretty --format progress --colors --no-interaction --verbose=2 --stop-on-failure $1"; }
dbehattag(){ dbash -c "php vendor/bin/behat --format pretty --format progress --colors --no-interaction --verbose=2 --stop-on-failure --tags='$1'"; }
